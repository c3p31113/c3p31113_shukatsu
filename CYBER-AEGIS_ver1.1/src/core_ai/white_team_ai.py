...
class WhiteTeamAI:
    ...
    def _sanitize_and_load_json(self, raw_text):
        try:
            ...
            print(f"Error: AI JSON response sanitization or parsing failed. Error: {e}")
            return None
    ...
    def run_full_simulation(self, sim_id):
        ...
        あなたはRed Teamの実行担当AIです。以下の作戦シナリオに基づき、ターゲットシステムで実行すべき具体的なコマンドを考案し、JSON形式で出力してください。
        【作戦シナリオ】
        {generic_red_team_prompt}
        【思考の証跡】
        ...
        {{
          ...
          "parameters": {{
            "command_to_execute": "（考案した具体的なコマンド）"
          }}
        }}
        ```
        """
        ...
        if not red_team_tactic or red_team_tactic.get("error"):
            ...
            final_report_summary = "Exercise complete: Red Team executed an attack based on the dynamic scenario, and Blue Team responded."
    ...
    def split_logs_for_display(self, action_history, arena_logs):
        return action_history, arena_logs

    def save_generated_module(self, sim_id, code_content):
        save_dir = "src/analyzers/generated"
