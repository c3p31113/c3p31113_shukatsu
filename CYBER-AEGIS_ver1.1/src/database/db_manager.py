...
class DBManager:
    ...
    def create_conversation(self, title):
        ...
        try:
            self.cursor.execute(query, (title, timestamp))
            self.conn.commit()
    ...
    def add_message_to_conversation(self, conv_id, message_data):
        ...
        try:
            self.cursor.execute(query, (conv_id, is_user_int, message_data['text'], timestamp))
            self.conn.commit()
        except sqlite3.Error as e:
    ...
    def update_conversation_title(self, conv_id, new_title):
        ...
        try:
            self.cursor.execute(query, (new_title, conv_id))
            self.conn.commit()
        except sqlite3.Error as e:
    ...
    def add_network_incident(self, incident_data):
        ...
        self.cursor.execute(query, (incident_data.get('id'), incident_data.get('name'), incident_data.get('time'), incident_data.get('destination'), incident_data.get('threat_level'), incident_data.get('status'), incident_data.get('description')))
        self.conn.commit()
    ...
    def add_file_event(self, event_data):
        ...
        self.cursor.execute(query, (event_data.get('id'), event_data.get('event_type'), event_data.get('path'), event_data.get('time'), event_data.get('threat_level'), event_data.get('description')))
        self.conn.commit()
    ...
    def add_github_leak(self, leak_data):
        ...
        self.cursor.execute(query, (leak_data.get('timestamp'), leak_data.get('source'), leak_data.get('keyword'), leak_data.get('repository'), leak_data.get('file_path'), leak_data.get('url'), json.dumps(leak_data.get('matches', []))))
        self.conn.commit()
    ...
    def add_x_leak(self, leak_data):
        ...
        self.cursor.execute(query, (leak_data.get('timestamp'), leak_data.get('source'), leak_data.get('keyword'), leak_data.get('author'), leak_data.get('tweet_text'), leak_data.get('url'), leak_data.get('tweet_created_at')))
        self.conn.commit()
        return self.cursor.lastrowid > 0
    ...
    def add_discord_leak(self, leak_data):
        ...
        self.cursor.execute(query, (leak_data.get('timestamp'), leak_data.get('source'), leak_data.get('keyword'), leak_data.get('server'), leak_data.get('channel'), leak_data.get('author'), leak_data.get('message_text'), leak_data.get('url')))
        self.conn.commit()
        return self.cursor.lastrowid > 0
    ...
    def update_leak_with_ai_analysis(self, unified_id, analysis_result):
        ...
        self.cursor.execute(query, (analysis_result.get('risk_level'), analysis_result.get('confidence'), report_str, leak_id))
        self.conn.commit()
    ...
    def update_leak_status(self, unified_id, status):
        ...
        self.cursor.execute(query, (status, leak_id))
        self.conn.commit()
    ...
    def update_community_score(self, server_id, server_name, invite_code, score, keywords, status):
        ...
        self.cursor.execute(query, (server_id, server_name, invite_code, score, timestamp, keywords_str, status))
        self.conn.commit()
    ...
    def get_prioritized_server_batch(self, all_invite_codes, batch_size=5):
        try:
            self.cursor.execute("SELECT invite_code, danger_score, last_analyzed_at, status FROM community_threat_scores")
            db_servers = {row[0]: {'score': row[1], 'last_scan': datetime.fromisoformat(row[2]), 'status': row[3]} for row in self.cursor.fetchall()}
    ...
    def get_total_event_counts(self):
        net_count = self.cursor.execute("SELECT COUNT(*) FROM network_incidents").fetchone()[0]
        file_count = self.cursor.execute("SELECT COUNT(*) FROM file_events").fetchone()[0]
    ...
    def _format_leak_rows(self, rows, source):
        ...
        for r in rows:
            ...
            try:
                row_dict = dict(r)
                if source == 'github':
                    leak_item = {'id': f"gh-{row_dict['id']}", **row_dict}
                    leak_item['matches'] = json.loads(row_dict['matches']) if row_dict['matches'] else []
                elif source == 'x':
                    leak_item = {'id': f"x-{row_dict['id']}", **row_dict}
                elif source == 'discord':
                    leak_item = {'id': f"dsc-{row_dict['id']}", **row_dict}
                elif source == 'pastebin':
                    leak_item = {'id': f"pst-{row_dict['id']}", **row_dict}
    ...
    def save_trinity_simulation(self, context, red_output, blue_output, white_report):
        ...
        """
        try:
            self.cursor.execute(query, (timestamp, context, red_output, blue_output, white_report))
            self.conn.commit()
            return self.cursor.lastrowid
        except sqlite3.Error as e:
    ...
    def add_system_learning(self, sim_id, learning_type, content):
        ...
        try:
            self.cursor.execute(query, (timestamp, sim_id, learning_type, content))
            self.conn.commit()
        except sqlite3.Error as e:
    ...
    def delete_trinity_simulation(self, sim_id):
        try:
            self.cursor.execute("DELETE FROM system_learnings WHERE source_simulation_id = ?", (sim_id,))
            self.cursor.execute("DELETE FROM trinity_ai_simulations WHERE id = ?", (sim_id,))
            self.conn.commit()
            return True
        except sqlite3.Error as e:
