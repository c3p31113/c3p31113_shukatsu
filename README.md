以下に自分が行ってきたプロジェクトや勉強の記録等の学習の過程についてまとめています。
...
* **成功事例：** パスワードが`12345678`やその他、人が設定しやすく推測されやすいパスワード解読に成功。この結果は、数字や良く設定されがちな文脈のみで構成された短いパスワードがいかに脆弱であるかを明確に示しています。
* **結論：** この実験は、不正アクセスの主要な原因の一つが、技術的なシステムの脆弱性だけでなく、**人間側のパスワード管理の甘さ**、つまりソーシャルエンジニアリングにあるという統計的事実 を、技術的に裏付けるものでした。攻撃者の視点を体験することで、パスワードの複雑化や定期的な変更といった、基本的な防御策の重要性を深く学びました。
...
![図１：SNS監視画面](./CYBER-AEGISスクリーンショット/SNS監視画面.png)
...
![図７：三位一体シミュレーション画面](./CYBER-AEGISスクリーンショット/三位一体シミュレーション画面.png)
...
本稿では、この方法論的欠陥を是正するため、\*\*厳格な科学的検証プロセス（[3. 方法論](#3-方法論-methodology)参照）\*\*を採用した。`IsolationForest`モデルを、公開データセット（`./Datasets`）から抽出した「正常」トラフィック（Honeypotスキャンデータ `n=24,262`）のみで訓練。評価には、訓練に一切使用していない「未知の」正常データ（`n=10,398`）と、複数の現代マルウェアファミリー（`n=78`）を混合した、**不均衡比 133:1** の現実的なテストセット（`n=10,476`）を構築した（`./Python/scientific_validator.py`）。

実験の結果（[4.1. ベースライン性能](#41-ベースライン性能の確立-phase-1)）、モデルは未知のマルウェアに対し `Recall: 0.60` を達成したが、`Precision: 0.04` となり、大量の誤報（False Positive, `n=1031`）を生成した。本研究の核心は、この結果を失敗として棄却するのではなく、`Recall 0.60` の内訳、すなわち「検知されたマルウェア群（True Positive, `n=47`）」と「見逃されたステルス群（False Negative, `n=31`）」を分離し、その決定境界を説明可能AI（XAI）のSHAPを用いて解剖（`./Python/phase4_xai_investigator.py`）した点にある。

分析（[4.3. XAIによる解剖](#43-xaiによるステルス・メカニズムの解剖-phase-4)）の結果、マルウェアの検知可否はファミリー名（例：RemcosRAT vs LummaStealer）とは統計的に**有意な相関がない**（マン・ホイットニーU検定, `p > 0.1`）ことが示された（[表2](#表2-マン・ホイットニーのu検定結果)）。むしろ、検知の成否を分けるのは個々のセッションの統計的特徴量であった。SHAP分析（[図4](#図4-見逃し理由ステルス群fn群のshap分析)）は、「見逃されたステルス群（`n=31`）」が、**`avg_iat`（低）**、**`direction_ratio`（高）**、\*\*`packet_count`（低）\*\*という3つの特徴量を、訓練データである「Honeypotノイズ」の統計分布に意図的に擬態（Mimesis）させることで、モデルの検知を回避していたことを定量的に明らかにした。
...
| **`./Results/`** | 分析プロセス（Phase 2-4）で生成された主要なグラフ（`.png`）。本READMEの[図2](#図2-異常スコア分布)〜[図4](#図4-見逃し理由ステルス群fn群のshap分析)で使用。 |
...
2.  **Phase 1 (検証):** `./Python/scientific_validator.py` を実行し、[表1](#表1-厳格なテストセットにおけるモデル性能)の性能値（Precision/Recall）を算出する。
3.  **Phase 2 (スコア化):** `./Python/phase2_score_analyzer.py` を実行し、`./Datasets/anomaly_scores_with_labels.csv` と `./Results/phase2_distribution_proof.png`（[図2](#図2-異常スコア分布)）を生成する。
4.  **Phase 3 (統計検定):** `./Python/phase3_statistical_prover.py` を実行し、[表2](#表2-マン・ホイットニーのu検定結果)のp値を算出する。
5.  **Phase 4 (XAI解剖):** `./Python/phase4_xai_investigator.py` を実行し、`./Results/phase4_shap_*.png`（[図3](#図3-検知理由tp群のshap分析), [図4](#図4-見逃し理由ステルス群fn群のshap分析)）を生成する。
...
モデルには `scikit-learn` の `IsolationForest`（`contamination='auto'`, `random_state=42`）を採用した。このモデルは、データに特定の分布を仮定せず、高次元データに対しても効率的に機能する教師なし手法であるため、本タスクに最適と判断した。
...
1.  **マルウェアは統計的に異常である:** マルウェア群は（サンプル数が `n=6` や `n=8` と極端に少なくても）、「正常」群とは統計的に明確に異なる（`p < 0.03`）振る舞いをしていることが証明された。
...
**1. 批判：「マルウェアサンプル数が僅少（`n=78`）である」**
  ...
  * **回答:** この事実は、本研究の結論の妥当性に影響しない。なぜなら、本研究は [表2] に示す通り、サンプル数が僅少なファミリー間の比較（例：Lumma `n=8` vs AgentTesla `n=6`）は統計的有意差なし（`p > 0.1`）として、**科学的誠実性に基づき棄却**しているからである。
...
**2. 批判：「"正常"の定義が偏っている（Honeypotノイズのみ）」**
  ...
  * 本研究の目的は「万能検知器」の構築ではなく、「**もし**AIが『Honeypotノイズ』という特定の統計分布を"正常"と学習**した場合**、マルウェアはどのような擬態戦術をとるか？」という基礎的な問いに答えるための対照実験（Controlled Experiment）である。
